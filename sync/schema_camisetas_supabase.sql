-- =========================================================================
-- ESQUEMA COMPLETO PARA EL PROYECTO SUPABASE (TIENDA DE KARY - CAMISETAS)
-- Generado para inicializar la base de datos Supabase en limpio (Proyecto 0).
-- =========================================================================
...
CREATE TABLE public.clientes (
  ...
  categoria text DEFAULT 'Normal'::text
);
...
CREATE TABLE public.pedidos (
  ...
  valor_domicilio real DEFAULT 0
);
...
-- 2. Constraints, Keys, Triggers y Funciones

ALTER TABLE ONLY public.abonos_credito ADD CONSTRAINT abonos_credito_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.abonos_pedido ADD CONSTRAINT abonos_pedido_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.config_sucursal ADD CONSTRAINT config_sucursal_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.detalle_pedidos ADD CONSTRAINT detalle_pedidos_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.detalle_transferencias ADD CONSTRAINT detalle_transferencias_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.inventario_sucursal ADD CONSTRAINT inventario_sucursal_pkey PRIMARY KEY (producto_id, sucursal_id);
...
ALTER TABLE ONLY public.solicitudes_venta ADD CONSTRAINT solicitudes_venta_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.solicitudes_gasto ADD CONSTRAINT solicitudes_gasto_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.transferencias ADD CONSTRAINT transferencias_pkey PRIMARY KEY (id);
...
CREATE INDEX idx_pedidos_fecha_entrega ON public.pedidos USING btree (fecha_entrega_estimada);
CREATE INDEX idx_ventas_fecha ON public.ventas USING btree (fecha);


-- Función guardiana LWW (Last-Write-Wins)
...
-- Función de asignación de secuencia incremental
CREATE OR REPLACE FUNCTION public.assign_sync_seq()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
begin
    new.sync_seq := nextval('sync_seq_sequence');
    return new;
end;
$function$
;

-- Secuencia para pull incremental
CREATE SEQUENCE IF NOT EXISTS sync_seq_sequence START 1;
...
-- 3. Habilitación de Row Level Security (RLS) y Políticas
...
CREATE POLICY solicitudes_gasto_all ON public.solicitudes_gasto FOR ALL TO anon,authenticated USING (true) WITH CHECK (true);
